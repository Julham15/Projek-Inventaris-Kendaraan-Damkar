...
<header class="flex justify-between items-center w-full px-lg py-md sticky top-0 z-40 bg-white border-b border-outline-variant">
    <div class="flex items-center gap-md">
        ...
        <span class="font-h2 text-h2 font-bold text-primary">Edit Jenis Mobil</span>
    </div>
...
<div class="p-lg md:p-margin flex-1 overflow-x-hidden">
    ...
    <div class="flex items-center gap-md mb-lg">
        ...
        <div>
            <h1 class="font-h1 text-h1 text-on-background">Edit Jenis Mobil</h1>
            <p class="font-body-regular text-body-regular text-on-surface-variant mt-1">Perbarui informasi kategori armada pemadam kebakaran.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-sm border border-[#D1E9F6] p-lg md:p-xl max-w-3xl">
        <form action="{{ route('posko.jenis-mobil.update', [$posko->id, $data->id]) }}" 
              method="POST" 
            ...
            <div class="space-y-xs">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-md">
                    ...
                    <div class="aspect-video md:aspect-square bg-surface-container border-2 border-dashed border-outline-variant rounded-lg flex flex-col items-center justify-center text-on-surface-variant overflow-hidden">
                        @if ($data->gambar)
                        <div class="w-full h-full" id="preview-container">
                            <img src="{{ asset('storage/' . $data->gambar) }}" 
                                 class="w-full h-full object-cover" 
                                 alt="Gambar {{ $data->nama_jenis }}">
                        </div>
                        @else
                    ...
                    <div class="md:col-span-2 relative">
                        <label class="flex flex-col items-center justify-center w-full h-full min-h-[120px] border-2 border-dashed border-outline-variant rounded-lg bg-surface-bright cursor-pointer hover:bg-[#E6F6FF] transition-colors px-md text-center @error('gambar') border-error @enderror" for="car_image">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="font-h4 text-h4 text-[#38B6FF] font-bold">Ganti Gambar</p>
                                <p class="font-caption text-caption text-on-surface-variant mt-xs">PNG, JPG atau WEBP (Maks. 2MB)</p>
                                @if ($data->gambar)
                                <p class="font-small text-small text-on-surface-variant mt-1">Gambar saat ini: {{ basename($data->gambar) }}</p>
                                @endif
                            </div>
                        ...
                        @enderror
                        @if ($data->gambar)
                        <p class="font-small text-small text-on-surface-variant mt-1 flex items-center gap-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-gutter p-md bg-[#E6F6FF]/20 rounded-lg border border-[#D1E9F6]">
                <div>
                    <label class="block font-small text-small text-on-surface-variant">ID Jenis Mobil</label>
                    <p class="font-body-regular text-body-regular text-on-surface">{{ $data->id ?? '-' }}</p>
                </div>
                <div>
                    <label class="block font-small text-small text-on-surface-variant">Tanggal Dibuat</label>
                    <p class="font-body-regular text-body-regular text-on-surface">{{ isset($data->created_at) ? $data->created_at->format('d M Y H:i') : '-' }}</p>
                </div>
            </div>
    ...
    <div class="mt-xl grid grid-cols-1 md:grid-cols-3 gap-gutter max-w-3xl">
        <div class="bg-[#E6F6FF]/20 p-md rounded-lg flex items-start gap-md border border-[#D1E9F6]">
            ...
            <div>
                ...
                <p class="font-small text-small text-on-surface-variant">Pastikan data jenis mobil diperbarui dengan benar.</p>
            </div>
        </div>
        <div class="bg-[#E6F6FF]/20 p-md rounded-lg flex items-start gap-md border border-[#D1E9F6]">
            ...
            <div>
                ...
                <p class="font-small text-small text-on-surface-variant">Upload gambar baru untuk mengganti gambar lama.</p>
            </div>
        </div>
        <div class="bg-[#E6F6FF]/20 p-md rounded-lg flex items-start gap-md border border-[#D1E9F6]">
